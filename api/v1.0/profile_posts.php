...
if (! empty($_GET['profileid']))
{
	...
	$getIdQuery = $conn->query("SELECT id FROM " . DB_ACCOUNTS . " WHERE id=$profileid AND type IN ('user','page')");
...
elseif (! empty($_GET['username']))
{
	...
	$getIdQuery = $conn->query("SELECT id FROM " . DB_ACCOUNTS . " WHERE username='$username' AND type IN ('user','page')");

...
if ($type == "search")
{
	if (! empty($_GET['query']))
	{
		...
		if (! empty($_GET['limit']))
		{
			...
			$profileQuery = $conn->query("SELECT id,about,avatar_id,cover_id,name,username,verified,type FROM " . DB_ACCOUNTS . " WHERE name LIKE '%$query%' AND active=1 AND type IN ('user','page') LIMIT 5");

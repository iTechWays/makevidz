    ...
    <div class="content-wrapper">
        ...
        <div class="input float-left">
            <select name="language">
                ...
                $langQuery = mysqli_query($dbConnect, "SELECT DISTINCT type FROM " . DB_LANGUAGES);
                
                while ($langFetch = mysqli_fetch_assoc($langQuery))
                {
                    $language = $langFetch['type'];
                    $language = str_replace('../languages/', '', $language);
                    $language = preg_replace('/([A-Za-z]+)\.php/i', '$1', $language);
                ?>
                <option value="<?php echo $language; ?>" <?php if ($language == $config['language']) echo 'selected'; ?>>
